/**
 * Financials Dashboard API Client
 *
 * Provides a clean interface to the backend financials endpoints.
 */
...
export const snapshotsApi = {
  ...
  /**
   * Generate financials snapshot
   */
  generateFinancials: async (recordedAt = null) => {
    const data = recordedAt ? { recorded_at: recordedAt } : {};
    const response = await api.post(
      `${BASE_URL}/financials-snapshots/generate/`,
      data
    );
    return response.data;
  },
