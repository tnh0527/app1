"""
Seed data for Financials Dashboard
...
from financials_app.models import (
    FinancialAccount,
    AccountSnapshot,
    FinancialsSnapshot,
    CashFlowEntry,
...
from financials_app.services import FinancialsService

User = get_user_model()


class Command(BaseCommand):
    help = 'Seed financials dashboard with demo data'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('Successfully seeded financials data!'))
    
    def clear_data(self, user):
        """Clear existing financials data for user."""
        self.stdout.write('Clearing existing data...')
        
        FinancialAccount.objects.filter(owner=user).delete()
        CashFlowEntry.objects.filter(owner=user).delete()
        FinancialsMilestone.objects.filter(owner=user).delete()
        FinancialsSnapshot.objects.filter(owner=user).delete()
    ...
    def create_milestones(self, user):
        ...
        for i, milestone_data in enumerate(milestones):
            FinancialsMilestone.objects.create(
                owner=user,
                display_order=i,
                **milestone_data
            )
    ...
    def generate_networth_snapshots(self, user):
        ...
        service = FinancialsService(user)
        today = date.today()
